...
public interface LikeListRepository extends JpaRepository<LikeList, Integer> {
    ...
    boolean existsByFeed_FeedIdAndUser_Id(Integer feedId, String id);

    // 피드의 전체 좋아요 수
    Long countByFeed_FeedId(Integer feedId);

    // 좋아요 상세 엔티티 조회 (삭제용)
    Optional<LikeList> findByFeed_FeedIdAndUser_Id(Integer feedId, String id);

    // 좋아요 삭제
    void deleteByFeed_FeedIdAndUser_Id(Integer feedId, String id);
}
