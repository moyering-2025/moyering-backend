...
public interface ReportService {
    // 신고 목록 조회 (검색 + 필터링)
    Page<ReportDto> getReports(String keyword, String reportType, String processStatus, Pageable pageable);

    // 신고 처리 (승인/반려) - 처리 버튼 용
    // 원래는 요청받는 데이터 dto를 따로 만드는게 좋으나, 본 프로젝트는 작으므로 ReportDto 사용
    void processReport(Integer reportId, ReportDto request) ;
}
